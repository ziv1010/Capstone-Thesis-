...
\begin{table}[H]
  ...
  \begin{tabular}{lp{4.0cm}p{8.0cm}}
    \toprule
    ...
    \multicolumn{3}{@{}l}{\textit{Mistral Outcome Labeller}} \\[2pt]
     & \texttt{case\_outcome\_label}
       & Categorical outcome from the appellant's perspective:
         \texttt{appellant\_won}, \texttt{appellant\_lost}, or
         \texttt{postponed\_or\_procedural}. \\
     & \texttt{case\_outcome\_score}
       & Numeric encoding of the label: $+1$ (won), $-1$ (lost), $0$
         (procedural). \\
     & \texttt{llm\_case\_outcome.confidence}
       & Model-reported classification confidence: \texttt{high},
         \texttt{medium}, or \texttt{low}. \\
     & \texttt{llm\_case\_outcome.short\_explanation}
...
\begin{table}[H]
  ...
  \begin{tabular}{llp{9.5cm}}
    ...
    Group & ID & Question \\
    \midrule
    \multirow{3}{*}{\parbox{2.5cm}{\textbf{A: Win signals}\\(appellant's\\perspective)}}
      & Q1 & Did the appellant win the case? \\
      & Q2 & Was the petition, appeal, or application allowed? \\
      & Q3 & Did the court rule in favour of the petitioner or appellant? \\[6pt]
    \multirow{3}{*}{\parbox{2.5cm}{\textbf{B: Loss signals}\\(appellant's\\perspective)}}
      & Q4 & Did the respondent win the case? \\
      & Q5 & Was the petition, appeal, or application dismissed? \\
      & Q6 & Did the court rule against the appellant? \\[6pt]
    \multirow{2}{*}{\parbox{2.5cm}{\textbf{C: Neutrality\\clause}}}
...
\begin{enumerate}
  \item \textbf{Summary preamble.} Regex applied to the
    \texttt{opennyai\_summary[`PREAMBLE']} string. This is fast and covers the
