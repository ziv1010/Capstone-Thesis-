...
\begin{sloppypar}
\begin{itemize}
  \item \path{case--has_preamble/has_facts/has_arguments--*text*}:
    attach each text node to its case.
  \item \path{case--has_petitioner_arguments/has_respondent_arguments/}\linebreak[4]%
    \path{has_other_lawyer_arguments--*text*}:
    attach party-specific argument blocks.
  \item \path{case--has_petitioner/has_respondent--*party*},
    \path{case--heard_in--court},
    \path{case--decided_by_bench--judge},
    \path{case--has_petitioner_lawyer/has_defence_lawyer/has_lawyer--*counsel*}.
  \item \path{arguments--cites_statute--statute},
    \path{arguments--cites_provision--provision},
    \path{arguments--cites_precedent--precedent}: the citation
    backbone, attached to the \textsc{Arguments} node rather than to
    the case so that the citation sits in its legal-discourse
    context.
  \item \path{provision--belongs_to_statute--statute}: stitches
    the provision hierarchy into the graph so that a citation to
    Section~482 is two hops from IPC, not disconnected from it.
  \item \path{petitioner_lawyer--citation--petitioner_arguments},
    \path{defence_lawyer--citation--respondent_arguments},
    \path{lawyer--citation--other_lawyer_arguments}: connect each
    counsel to the argument block they are responsible for.
  \item \path{petitioner--is_party_in_arguments--petitioner_arguments},
    \path{respondent--is_party_in_arguments--respondent_arguments}:
...
through \texttt{torch\_geometric.}\allowbreak\texttt{to\_undirected} so that HGT's typed
...
\texttt{statute--used\_in\_}\allowbreak\texttt{arguments--arguments},
\texttt{judge--presided\_}\allowbreak\texttt{arguments--arguments},
\texttt{petitioner/respondent--}\allowbreak\texttt{is\_party\_in\_arguments--arguments} (to
the \emph{shared}, un-partitioned \textsc{Arguments} node). The updated
policy \emph{forbids} these edges (\path{FORBIDDEN_EDGE_TYPES}).
...
\begin{figure}[htbp]
    ...
    \caption{{Sample star-graph structure for a single Case node. Various text blocks feature as text nodes, while details like Court and Judge form local entity nodes. The Arguments node branches out to cite shared legal concepts like Statutes and Precedents.}}
...
\begin{figure}[htbp]
    ...
    \caption{{Cross-connection of cases via shared nodes. Each case maintains its own local representations for identity entities to prevent shortcut leakage. Shared nodes like Statutes and Precedents act as legally-sound cross-case conduits for message passing.}}
...
set of \path{FORBIDDEN_TOP_LEVEL_FIELDS} from the case JSON
...
\path{FORBIDDEN_ANNOTATION_LABELS} set
