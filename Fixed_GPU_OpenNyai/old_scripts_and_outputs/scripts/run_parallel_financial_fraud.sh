#!/bin/bash
# Parallel pipeline: financial_fraud on GPU 7
# Runs all 17 chunks with max 6 concurrent processes (6 x ~6 GiB = 36 GiB VRAM, safe on 49 GiB GPU)

set -euo pipefail

INPUT_BASE="/scratch/ziv_baretto/Thesis_Ziv/Capstone-Thesis-/DATA_SET_BUILDER_AND_EXPLORER/Bucket_Maker_Nyaya/opennyai_txt_export/output/CJPE_ext_SCI_HCs_Tribunals_daily_orders_test/financial_fraud"
OUTPUT_DIR="/scratch/ziv_baretto/Thesis_Ziv/Capstone-Thesis-/Fixed_GPU_OpenNyai/outputs/financial_fraud"
SCRIPT="/scratch/ziv_baretto/Thesis_Ziv/Capstone-Thesis-/Fixed_GPU_OpenNyai/run_pipeline.py"
LOG_DIR="${LOG_DIR:-/tmp/ff_chunk_logs}"
...
echo "[$(date)] Starting financial_fraud parallel run on GPU $GPU_ID"
...
wait
echo "[$(date)] financial_fraud DONE. Chunk logs in $LOG_DIR"
