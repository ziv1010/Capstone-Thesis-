...
OUTPUT_BASE="${OUTPUT_BASE:-$APP_ROOT/outputs/entity_resolved_section_sep_lr_decay_cross_bucket}"
...
usage() {
  cat <<'EOF'
Usage:
  FINAL_EXPLANATION/run_scripts/run_entity_resolved_section_sep_lr_decay_cross_bucket_all.sh [options]
...
if [[ "$RUN_IDENTITY_AUDIT" -eq 1 ]]; then
  ...
  "${PYTHON[@]}" "$APP_ROOT/identity_shortcut_audit.py" \
    --explanation-dir "$EXPLAIN_DIR" \
    --graph-cache "$GRAPH" \
    --predictions-csv "$PRED" \
    --output-dir "$EXPLAIN_DIR" \
...
if [[ "$RUN_PATTERNS" -eq 1 ]]; then
  ...
  "${PYTHON[@]}" "$APP_ROOT/structural_why_analysis.py" \
    --explanation-dir "$EXPLAIN_DIR" \
    --graph-cache "$GRAPH" \
    --predictions-csv "$PRED" \
  ...
  CUDA_VISIBLE_DEVICES="${GPUS%%,*}" "${PYTHON[@]}" "$APP_ROOT/extract_hgt_case_embeddings.py" \
    --explanation-dir "$EXPLAIN_DIR" \
    --graph-cache "$GRAPH" \
    --predictions-csv "$PRED" \
    --config "$CONFIG" \
  ...
  "${PYTHON[@]}" "$APP_ROOT/counterfactual_neighborhoods.py" \
  ...
  "${PYTHON[@]}" "$APP_ROOT/embedding_cluster_characterization.py" \
    --pattern-dir "$PATTERN_DIR" \
    --output-dir "$PATTERN_DIR" \
    --min-cluster-size 80 \
    --n-jobs -1
...
if [[ "$RUN_MASK_SENSITIVITY_AUDIT" -eq 1 ]]; then
  ...
  CUDA_VISIBLE_DEVICES="${GPUS%%,*}" "${PYTHON[@]}" "$APP_ROOT/mask_sensitivity_audit.py" \
    --explanation-dir "$EXPLAIN_DIR" \
