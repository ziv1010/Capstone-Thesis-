...
OUTPUT_DIR="$APP_ROOT/outputs/target_fold_00_multigpu"
SPLIT="test"
RUN_MERGE=1
...
usage() {
  cat <<'EOF'
Usage:
  FINAL_EXPLANATION/run_scripts/run_multi_gpu.sh [options] [-- extra explain_hgt.py args]
...
Examples:
  ...
  FINAL_EXPLANATION/run_scripts/run_multi_gpu.sh --gpus 0,1 --output-dir FINAL_EXPLANATION/outputs/smoke_multigpu -- --case-limit 2 --progress-every 1

  # Full fold-00 test run on GPUs 0-4.
  FINAL_EXPLANATION/run_scripts/run_multi_gpu.sh --gpus 0,1,2,3,4 --output-dir FINAL_EXPLANATION/outputs/target_fold_00_multigpu

  # Override the model/graph artifacts.
  FINAL_EXPLANATION/run_scripts/run_multi_gpu.sh --gpus 0,1 --output-dir FINAL_EXPLANATION/outputs/my_model -- \
...
if [[ "$RUN_MERGE" -eq 1 ]]; then
  "${PYTHON[@]}" "$APP_ROOT/merge_outputs.py" \
