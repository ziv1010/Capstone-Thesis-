...
EXPLANATION_DIR="$SCRIPT_DIR/outputs/target_fold_00_8gpu"
...
usage() {
  cat <<'EOF'
Usage:
  FINAL_EXPLANATION/run_validation_multi_gpu.sh [options] [-- extra validate_explanations.py args]
...
Examples:
  ...
  FINAL_EXPLANATION/run_validation_multi_gpu.sh --gpus 0,1 --output-dir FINAL_EXPLANATION/outputs/validation_smoke -- \
  ...
  FINAL_EXPLANATION/run_validation_multi_gpu.sh --gpus 0,1,2,3,4,5,6,7 \
...
for shard_idx in "${!GPU_ARRAY[@]}"; do
  ...
  (
    CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="$gpu" \
      "${PYTHON[@]}" "$SCRIPT_DIR/validate_explanations.py" \
...
if [[ "$RUN_MERGE" -eq 1 ]]; then
  "${PYTHON[@]}" "$SCRIPT_DIR/merge_validation_outputs.py" \
