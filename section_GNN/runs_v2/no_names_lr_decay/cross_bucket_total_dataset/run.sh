#!/usr/bin/env bash
# no_names_lr_decay — cross_bucket_total_dataset
# No-names ablation on the v2 graph (build_graph_v2.py / case_star_builder_v2.py).
...
RUN_NAME="cross_bucket_no_names_lr_decay_kfold"
K=5
...
if [[ "$SKIP_BUILD" == false ]]; then
  echo ">>> Building no-names v2 graph (cross_bucket) ..."
  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="$GPUS" \
    $PY "$PARTY_ARGS_DIR/graph/build_graph_v2.py" --config "$CONFIG"
...
for fold_idx in $(seq 0 $((K - 1))); do
  ...
  (CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="$fold_idx" \
    micromamba run -n "$MAMBA_ENV" python "$PARTY_ARGS_DIR/scripts/kfold_cv_v2.py" \
...
echo ">>> Aggregating ..."
micromamba run -n "$MAMBA_ENV" python "$PARTY_ARGS_DIR/scripts/kfold_cv_v2.py" \
  --config "$CONFIG" --run-name "$RUN_NAME" --k "$K" \
