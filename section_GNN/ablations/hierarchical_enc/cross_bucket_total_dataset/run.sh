...
RUN_NAME="${RUN_NAME:-ablation_hierarchical_enc_cross_bucket_kfold}"
K="${K:-5}"
...
for fold_idx in $(seq 0 $((K - 1))); do
  ...
  (
    CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="$fold_idx" PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
