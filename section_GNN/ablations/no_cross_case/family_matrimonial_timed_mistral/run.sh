#!/usr/bin/env bash
# Ablation: no cross-case sharing — all statute/provision/precedent nodes are local.
...
RUN_NAME="ablation_no_cross_case_family_matrimonial_kfold"
K=5
...
if [[ "$SKIP_BUILD" == false ]]; then
  echo ">>> Building ablation graph (no cross-case sharing) ..."
