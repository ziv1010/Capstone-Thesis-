...
# Matches a 4-digit year in the range 1800–2099 as a whole word.
_YEAR_RE = re.compile(r"\b(1[89]\d{2}|20\d{2})\b")


def _extract_year(text: str) -> int | None:
    """Return the first plausible legal year found in *text*, or None."""
    m = _YEAR_RE.search(text)
    return int(m.group(1)) if m else None


def _time_safe(entity_type: str, entity_text: str, case_year: int | None) -> bool:
    """Return True if the citation edge is temporally valid.

    Rules
    -----
    precedent : precedent_year < case_year  (cited case must predate the citing case)
    statute   : statute_year  <= case_year  (act must be enacted on or before the case)
    provision : same as its parent statute — use statute_year <= case_year when
                a year can be extracted from the provision text, otherwise allow.
    other     : always allow.

    If case_year is unknown, or the entity year cannot be parsed, allow the edge
    (conservative default — better to include than to silently drop).
    """
    ...
    if build_mode != "text_only":
        for entity in cleaned_case.entities:
            ...
            if arguments_node_key and entity.seen_in_arguments:
                ...
                if citation_relation and _time_safe(
                    entity.entity_type, entity.canonical_name, case_year
                ):
                    edges.append(
                        GraphEdge(
                            src_type="arguments",
                    ...
                    # Also route citation to the role-specific argument node when
                    # first_seen_section pins the entity to a particular role section.
                    role_key = role_argument_node_keys.get(entity.first_seen_section)
                    if role_key:
                        edges.append(
                            GraphEdge(
                                src_type=entity.first_seen_section,
                                relation=citation_relation,
                                dst_type=entity.entity_type,
                                src_key=role_key,
                                dst_key=key,
