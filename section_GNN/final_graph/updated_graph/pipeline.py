...
def build_graph_bundle(
    ...
    debug_samples = []
    debug_pairs = list(zip(prepared.cases, case_graphs))
    if debug_sample_size > 0 and debug_pairs:
        if debug_sample_size >= len(debug_pairs):
            selected_pairs = debug_pairs
        else:
            rng = random.Random(int(cfg.get("project", {}).get("seed", 42)))
            selected_indices = sorted(rng.sample(range(len(debug_pairs)), debug_sample_size))
            selected_pairs = [debug_pairs[idx] for idx in selected_indices]
    else:
        selected_pairs = []

    for case, case_graph in selected_pairs:
