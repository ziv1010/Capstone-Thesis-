...
def train_model(
    ...
    use_early_stopping = bool(cfg.get("training", {}).get("use_early_stopping", True))
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_metrics = evaluate_split(
            y_true=y_case[train_mask].detach().cpu().numpy(),
            y_pred=predictions[train_mask].detach().cpu().numpy(),
            y_proba=probabilities[train_mask].detach().cpu().numpy(),
        ...
        if logger is not None and (epoch == 1 or epoch % log_every == 0):
            logger.info(
                "epoch=%d/%d train_loss=%.4f train_macro_f1=%.4f val_macro_f1=%.4f best_val_macro_f1=%.4f patience_counter=%d",
                epoch,
                epochs,
                float(loss.item()),
                train_macro_f1,
