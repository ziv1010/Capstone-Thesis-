...
_OTHER_LAWYER_SIGNALS = {
    "learned counsel", "learned senior counsel", "counsel submitted", "counsel argued",
    "advocate submitted", "advocate argued", "counsel contended", "submitted that",
    "argued that", "contended that", "appearing for", "appeared for",
}
...
def _infer_argument_bucket(annotation: dict[str, Any], raw_text: str) -> str | None:
    annotation_text = normalize_whitespace(str(annotation.get("text", "") or ""))
    if not annotation_text:
        return None

    lawyer_sides: set[str] = set()
    for entity in annotation.get("entities", []) or []:
        labels = {str(label).upper() for label in entity.get("labels", []) or []}
        if "LAWYER" not in labels:
            continue
        lawyer_sides.add(
            _infer_lawyer_side(
                raw_text=raw_text,
    ...
    if "petitioner_lawyer" in lawyer_sides:
        return "petitioner_arguments"
    if "defence_lawyer" in lawyer_sides:
        return "respondent_arguments"

    lowered = annotation_text.lower()
    if any(signal in lowered for signal in _PETITIONER_SIGNALS):
        return "petitioner_arguments"
    if any(signal in lowered for signal in _RESPONDENT_SIGNALS):
        return "respondent_arguments"
    if "lawyer" in lawyer_sides or any(signal in lowered for signal in _OTHER_LAWYER_SIGNALS):
        return "other_lawyer_arguments"
    return None


def _aggregate_role_argument_texts(
    annotations: list[dict[str, Any]],
    raw_text: str,
    patterns: list[Any],
    mask_token: str,
    ...
    for annotation in annotations:
        if _annotation_section(annotation) != "arguments":
            continue
        bucket = _infer_argument_bucket(annotation, raw_text=raw_text)
        if bucket is None:
            continue
        text = normalize_whitespace(str(annotation.get("text", "") or ""))
        if not text:
        ...
        cleaned_text, matches = remove_or_mask_leakage(text, patterns, mask_token=mask_token)
        if cleaned_text:
            grouped_chunks[bucket].append(cleaned_text)
        grouped_matches[bucket].extend(matches)

    texts = {
        key: "\n".join(dict.fromkeys(values))
        for key, values in grouped_chunks.items()
    }
    return texts, grouped_matches
...
def extract_prejudgment_text(
    ...
    if bool(cfg.get("derive_role_argument_sections", True)):
        role_texts, role_matches = _aggregate_role_argument_texts(
            annotations=annotations,
            raw_text=raw_text,
            patterns=patterns,
            mask_token=mask_token,
        )
        for section_name, section_text in role_texts.items():
            retained_texts[section_name] = normalize_whitespace(section_text)
            if section_text:
                text_audit["sources"][section_name] = "annotation_bucket:arguments"
            text_audit["masked_matches"][section_name].extend(role_matches.get(section_name, []))
