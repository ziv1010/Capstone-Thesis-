...
def apply_reasoning_graph_policy(graph_cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    shareable_node_types = list(
        cfg.get("shareable_node_types")
        or (
            "court",
            "judge",
            "petitioner_lawyer",
    ...
    cfg["shareable_node_types"] = [
        node_type
        for node_type in _ordered_unique(shareable_node_types)
        if node_type in UPDATED_ENTITY_NODE_TYPES and node_type not in {"petitioner", "respondent"}
    ]
